...
服务端**只念模型的消息**：拿用户消息去合成，`finish` 会带 `code=6 NO_CONTENT`。

所以 `say` 发进会话的不是原文，而是一句「把下面这句话原样复述一遍 + 原文」的提示语，
然后等模型那条回复，拿它的 `message_id` 去合成。念出来的是模型复述的内容。

早先版本还试过「直接念用户消息」（当时的 `via=user`）和「先试 user 再自动换 reply」
（`via=auto`），实测确认前者必然被拒，后者的自动回退只是白烧一个会话 + 一次 PoW，
两个都去掉了。现在只有一条路。
...
12. **`difficulty` 的真实量级。** 服务端给多大的上界完全未知（本地合成题只用到几千）。
    本包默认上限 500 万次迭代，超了会明确报错而不是无限跑。
13. **`role` 的枚举值。** 只知道客户端写的是 `role === MessageRole.ASSISTANT ? ASSISTANT : USER`，
    具体数字/字符串没记。所以选消息时**不依赖 role**，改用「正文非空、且不等于我发进去的那条」这个判据。
14. **是否还需要 `x-hif-*` 之类的指纹头。** 本包不发，见上一节的说明。
15. **`chat/completion` 的 SSE 事件结构。** 本包不用：把流读干再看 history，
    所以事件名/字段一个都没用到，也就不需要它们对。
16. **删会话会不会因为"会话里正在生成"而失败。** 本包先读完流再删，没在真机上确认过顺序够不够。

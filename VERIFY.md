...
> **读的时候注意**：这份是按时间顺序记的，**写完之后代码又变过**，所以里面的命令和输出
> 不一定还是现在的样子。已知的出入：
>
> - `--via user|reply|auto` **已经删掉了**。当时试出"服务端不念用户消息"，所以留下来的是
>   reply 一条路；`user` 必然被拒、`auto` 只是白烧一个会话+一次 PoW，两个都去掉了。
>   现在 `say` 没有这个开关，底下正文里出现 `via=...` 的地方都是当时的记录。
> - `voices` 和 `demo` 现在**要求登录态**。当时允许不带 token 退回内置表，后来发现这会让
>   人拿着过期的表来问为什么对不上，就改成必须有 token 了。所以下面「不需要登录」那两节是旧的。
> - 测试用例数从 86 → 126 → **124**（删掉两个随 `--via` 一起废掉的用例）。

**顶要紧的一句先放前面**：写第一、二轮的时候没有 token，所以那些"需要登录态的路径没跑过"
的结论，**在后面「真账号端到端实测」一节里已经被推翻了** —— 那一节是真跑通的。
看结论直接跳到那一节。
